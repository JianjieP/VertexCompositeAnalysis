...
// 1. Mass difference minimization A: larger difference
// 2. pT difference minimization A: smaller
// 3. pT sum maximization A: smaller
// 4. Random pairing
//
...
void twopairs() {
    ...
    bool generate_individual_pdf = false;   // Toggle individual-per-plot PDF outputs
    bool generate_combined_pdf = true;     // Toggle combined multi-page PDFs per method
    bool generate_png = false;             // Toggle PNG outputs
    
    std::cout << "=== Output Generation Settings ===" << std::endl;
    std::cout << "Individual PDF generation: " << (generate_individual_pdf ? "ENABLED" : "DISABLED") << std::endl;
    std::cout << "Combined PDF generation: " << (generate_combined_pdf ? "ENABLED" : "DISABLED") << std::endl;
    ...
    std::array<bool,4> method_enabled = { true, true, true, true };
    ...
    if (!fin || fin->IsZombie()) {
        std::cerr << "Error: cannot open filtered_chained_ParticleTree.root" << std::endl;
        return;
    }
    ...
    // Clean up existing PNG files before generating new ones (controlled by clean_png_before)
    if (generate_png && clean_png_before) {
        std::cout << "Cleaning up existing PNG files..." << std::endl;
        try {
    ...
    if (generate_png || generate_individual_pdf || generate_combined_pdf) {
    ...
    auto createPlot = [&](const std::vector<TH1F*>& hists, const std::string& title, 
        ...
        if (generate_png) c->SaveAs((filename + ".png").c_str());
        if (generate_individual_pdf) c->SaveAs((filename + ".pdf").c_str());
        delete c;
    };
    ...
    auto createCombinedPDF = [&](size_t method_idx) {
        ...
        combined_canvas->cd();
        auto legend = new TLegend(0.68, 0.72, 0.88, 0.88);
        legend->SetBorderSize(0);
        legend->SetFillStyle(0); // Transparent to avoid obscuring plots
    ...
    for (size_t i = 0; i < methods.size(); ++i) {
        ...
        if (generate_combined_pdf && anyToPlot) {
            createCombinedPDF(i);
        }

        if ((generate_png || generate_individual_pdf) && anyToPlot) {
    ...
    if ((generate_png || generate_individual_pdf) && save_four_vectors) {
    ...
    if (generate_combined_pdf && save_four_vectors) {
        auto histNonEmpty = [&](TH1F* h) { return (h && h->GetEntries() > 0); };
        if (histNonEmpty(h_four_pi_mass) || histNonEmpty(h_four_pi_pT) || histNonEmpty(h_individual_pT)) {
    ...
    if (generate_combined_pdf) outputs.push_back("combined PDFs (per method)");
    if (generate_individual_pdf) outputs.push_back("individual plot PDFs");
    if (generate_png) outputs.push_back("PNG plots");
    if (outputs.empty()) {
        output_summary += "ROOT histograms only (no plots generated)";
    } else {
        for (size_t i = 0; i < outputs.size(); ++i) {
            if (i > 0) output_summary += " + ";
            output_summary += outputs[i];
        }
    }
