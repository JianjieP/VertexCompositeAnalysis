// Standalone extraction of the rho-closest x-frame pion angle plots with total-pT cuts.
...
void saveAnVsPtPlot(const std::vector<double> &ptCenters,
    ...
    TH1F frame(Form("h_frame_a%d", harmonic), Form(";total p_{T};A_{%d}", harmonic), 100, 0.0, 1.00);
...
void saveAnVsPtNeutronPlot(const std::vector<double> &ptCenters,
    ...
    TH1F frame(Form("h_frame_a%d_neutron_%s", harmonic, isBest ? "closest" : "counterpart"),
               Form(";total p_{T};A_{%d}", harmonic), 100, 0.0, 1.00);
    frame.SetMinimum(yMin - margin);
    frame.SetMaximum(yMax + margin);
    frame.SetStats(0);
    frame.Draw();
...
void fourpair_rho_x_totalptcut(const char *input_path = "/eos/user/j/jianjie/fourpi/HIRun2025A_FourPi_0410/filtered_chained_ParticleTree.root",
                              const char *output_root = "fourpair_rho_x_totalptcut.root",
                          bool generate_png = true,
                          bool generate_pdf = false) {
    const bool export_pion_angles_rho_x_totalptcut = false;
    const bool export_pion_angles_rho_x_totalptcut_mid_high = false;
    ...
    const std::string png_dir = "fourpair_rho_x_totalptcut_PNG";
    const std::string a1234_dir = png_dir + "/a1234_closest_counterpart_total_pt";
    const std::string a1234_neutron_dir = png_dir + "/a1234_neutron_selection_total_pt";
    ...
    for (Long64_t i = 0; i < nentries; ++i) {
        ...
        bool passPtCut = export_pion_angles_rho_x_totalptcut && totalPt >= 0.0 && totalPt < 0.1;
        bool passPtCutMid = export_pion_angles_rho_x_totalptcut && export_pion_angles_rho_x_totalptcut_mid_high && totalPt >= 0.1 && totalPt < 0.2;
        bool passPtCutHigh = export_pion_angles_rho_x_totalptcut && export_pion_angles_rho_x_totalptcut_mid_high && totalPt >= 0.2 && totalPt < 1.0;

        if (passPtCut) {
            h_pi_phi_x_rho_best_pt->Fill(piPhiX[bestIdx]);
            h_pi_phi_x_rho_other_pt->Fill(piPhiX[otherPair]);
        }
        if (totalPt >= finePtMin && totalPt < finePtMax) {
            int fineBin = static_cast<int>((totalPt - finePtMin) / finePtStep);
    ...
    if (export_pion_angles_rho_x_totalptcut) {
        savePhiPlot(h_pi_phi_x_rho_best_pt,
                "c_pi_phi_x_rho_best_totalpt",
                "Pion #Delta#varphi (closest #rho, total p_{T}<0.1)",
                "#pi^{+} #Delta#varphi (closest #rho, total p_{T}<0.1)",
                png_dir + "/pi_phi_x_rho_best_totalpt.png",
                "pi_phi_x_rho_best_totalpt.pdf",
                    generate_png,
                    generate_pdf,
                    kBlue + 2,
                    fitOrder,
        ...
        savePhiPlot(h_pi_phi_x_rho_other_pt,
                "c_pi_phi_x_rho_other_totalpt",
                "Pion #Delta#varphi (counterpart, total p_{T}<0.1)",
                "#pi^{+} #Delta#varphi (counterpart, total p_{T}<0.1)",
                png_dir + "/pi_phi_x_rho_other_totalpt.png",
                "pi_phi_x_rho_other_totalpt.pdf",
                    generate_png,
                    generate_pdf,
                    kMagenta + 2,
                    fitOrder,
    ...
    if (export_pion_angles_rho_x_totalptcut_mid_high) {
        savePhiPlot(h_pi_phi_x_rho_best_pt_mid,
                "c_pi_phi_x_rho_best_totalpt_mid",
                "Pion #Delta#varphi (closest #rho, 0.1<total p_{T}<0.2)",
                "#pi^{+} #Delta#varphi (closest #rho, 0.1<total p_{T}<0.2)",
                png_dir + "/pi_phi_x_rho_best_totalpt_0p1_0p2.png",
                "pi_phi_x_rho_best_totalpt_0p1_0p2.pdf",
                    generate_png,
                    generate_pdf,
                    kBlue + 2,
                    fitOrder,
        ...
        savePhiPlot(h_pi_phi_x_rho_best_pt_high,
                "c_pi_phi_x_rho_best_totalpt_high",
                "Pion #Delta#varphi (closest #rho, total p_{T}>0.2)",
                "#pi^{+} #Delta#varphi (closest #rho, total p_{T}>0.2)",
                png_dir + "/pi_phi_x_rho_best_totalpt_gt_0p2.png",
                "pi_phi_x_rho_best_totalpt_gt_0p2.pdf",
                    generate_png,
                    generate_pdf,
                    kBlue + 2,
                    fitOrder,
        ...
        savePhiPlot(h_pi_phi_x_rho_other_pt_mid,
                "c_pi_phi_x_rho_other_totalpt_mid",
                "Pion #Delta#varphi (counterpart, 0.1<total p_{T}<0.2)",
                "#pi^{+} #Delta#varphi (counterpart, 0.1<total p_{T}<0.2)",
                png_dir + "/pi_phi_x_rho_other_totalpt_0p1_0p2.png",
                "pi_phi_x_rho_other_totalpt_0p1_0p2.pdf",
                    generate_png,
                    generate_pdf,
                    kMagenta + 2,
                    fitOrder,
        ...
        savePhiPlot(h_pi_phi_x_rho_other_pt_high,
                "c_pi_phi_x_rho_other_totalpt_high",
                "Pion #Delta#varphi (counterpart, total p_{T}>0.2)",
                "#pi^{+} #Delta#varphi (counterpart, total p_{T}>0.2)",
                png_dir + "/pi_phi_x_rho_other_totalpt_gt_0p2.png",
                "pi_phi_x_rho_other_totalpt_gt_0p2.pdf",
                    generate_png,
                    generate_pdf,
                    kMagenta + 2,
                    fitOrder,
    ...
    std::ofstream bestTxt(png_dir + "/pi_phi_x_rho_best_totalpt_fits.txt");
    std::ofstream otherTxt(png_dir + "/pi_phi_x_rho_other_totalpt_fits.txt");
    ...
    for (int bin = 0; bin < finePtBins; ++bin) {
        ...
        std::string bestCanvas = Form("c_pi_phi_x_rho_best_totalpt_fine_%s", binTag.c_str());
        std::string otherCanvas = Form("c_pi_phi_x_rho_other_totalpt_fine_%s", binTag.c_str());
        std::string bestTitle = Form("Pion #Delta#varphi (closest #rho, %.2f<=total p_{T}<%.2f)", low, high);
        std::string otherTitle = Form("Pion #Delta#varphi (counterpart, %.2f<=total p_{T}<%.2f)", low, high);
        std::string bestLegend = Form("#pi^{+} #Delta#varphi (closest #rho, %.2f<=total p_{T}<%.2f)", low, high);
        std::string otherLegend = Form("#pi^{+} #Delta#varphi (counterpart, %.2f<=total p_{T}<%.2f)", low, high);
        std::string note = Form("%.2f<=total p_{T}<%.2f", low, high);

        savePhiPlot(h_pi_phi_x_rho_best_pt_fine[bin],
                    bestCanvas.c_str(),
                    bestTitle.c_str(),
                    bestLegend.c_str(),
                    png_dir + "/pi_phi_x_rho_best_totalpt_" + lowHighTag + ".png",
                    "pi_phi_x_rho_best_totalpt_" + lowHighTag + ".pdf",
                    generate_png,
                    generate_pdf,
                    kBlue + 2,
                    fitOrder,
        ...
        savePhiPlot(h_pi_phi_x_rho_other_pt_fine[bin],
                    ...
                    otherTitle.c_str(),
                    otherLegend.c_str(),
                    png_dir + "/pi_phi_x_rho_other_totalpt_" + lowHighTag + ".png",
                    "pi_phi_x_rho_other_totalpt_" + lowHighTag + ".pdf",
                    generate_png,
                    generate_pdf,
                    kMagenta + 2,
                    fitOrder,
        ...
        savePhiPlotWithNeutronStates(h_pi_phi_x_rho_best_pt_fine_by_neutron[bin],
                                     Form("c_pi_phi_x_rho_best_totalpt_neutron_%s", binTag.c_str()),
                                     Form("Pion #Delta#varphi (closest #rho, neutron split, %.2f<=total p_{T}<%.2f)", low, high),
                                     png_dir + "/pi_phi_x_rho_best_totalpt_neutron_" + lowHighTag + ".png",
                                     "pi_phi_x_rho_best_totalpt_neutron_" + lowHighTag + ".pdf",
                                     generate_png,
                                     generate_pdf,
                                     true,
                                     fitOrder,
        ...
        savePhiPlotWithNeutronStates(h_pi_phi_x_rho_other_pt_fine_by_neutron[bin],
                                     Form("c_pi_phi_x_rho_other_totalpt_neutron_%s", binTag.c_str()),
                                     Form("Pion #Delta#varphi (counterpart, neutron split, %.2f<=total p_{T}<%.2f)", low, high),
                                     png_dir + "/pi_phi_x_rho_other_totalpt_neutron_" + lowHighTag + ".png",
                                     "pi_phi_x_rho_other_totalpt_neutron_" + lowHighTag + ".pdf",
                                     generate_png,
                                     generate_pdf,
                                     false,
                                     fitOrder,
    ...
    for (int harmonic = 1; harmonic <= 4; ++harmonic) {
        saveAnVsPtPlot(ptCenters,
                       ...
                       anOtherErr[harmonic],
                       harmonic,
                       a1234_dir + Form("/a%d_vs_total_pt_closest_counterpart.png", harmonic),
                       Form("a%d_vs_total_pt_closest_counterpart.pdf", harmonic),
                       generate_png,
                       generate_pdf);
        ...
        saveAnVsPtNeutronPlot(ptCenters,
                              ...
                              harmonic,
                              true,
                              a1234_neutron_dir + Form("/a%d_vs_total_pt_closest_neutron.png", harmonic),
                              Form("a%d_vs_total_pt_closest_neutron.pdf", harmonic),
                              generate_png,
                              generate_pdf);
        ...
        saveAnVsPtNeutronPlot(ptCenters,
                              ...
                              harmonic,
                              false,
                              a1234_neutron_dir + Form("/a%d_vs_total_pt_counterpart_neutron.png", harmonic),
                              Form("a%d_vs_total_pt_counterpart_neutron.pdf", harmonic),
                              generate_png,
                              generate_pdf);
    }
