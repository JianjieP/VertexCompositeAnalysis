...
void sampleAna_chaining_EmptyBX() {
    // Create a TChain and add your ROOT files.
    // EmptyBX files store the tree under eventAna/ParticleTree.
    TChain* chain = new TChain("eventAna/ParticleTree");
    ...
    TString findCmd = "find /eos/user/j/jianjie/fourpi -type f "
                      "-path '*/HIEmptyBX/HIEmptyBX/*/*' "
                      "-name 'ana_EmptyBX_*.root' 2>/dev/null || true";
    ...
    cout << "Found " << totalFiles << " candidate files in HIEmptyBX directories. Checking for valid trees..." << endl;
    ...
    for (Int_t i = 0; i < totalFiles; i++) {
        ...
        if (testFile && !testFile->IsZombie()) {
            TDirectoryFile* dir = (TDirectoryFile*)testFile->Get("eventAna");
            if (dir) {
    ...
    TString outputFileName = "/eos/user/j/jianjie/fourpi/HIRun2025A_FourPi_0410/chained_ParticleTree_EmptyBX.root";
