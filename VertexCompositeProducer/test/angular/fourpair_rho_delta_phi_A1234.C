...
TLorentzVector boostToTotalRestFrame(const TLorentzVector &pion, const TLorentzVector &total) {
    TLorentzVector boosted = pion;
    boosted.Boost(-total.BoostVector());
    return boosted;
}
...
void fourpair_rho_delta_phi_A1234(const char *input_path = "/eos/user/j/jianjie/fourpi/HIRun2025A_FourPi_0410/filtered_chained_ParticleTree.root",
    ...
    const std::string outputDir4pi = "fourpair_rho_delta_phi_A1234_PNG_4piRF";
    std::filesystem::create_directories(outputDir);
    std::filesystem::create_directories(outputDir4pi);
    ...
    TH1F *h_deltaPhi4pi_all = new TH1F("h_deltaPhi4pi_all", ";#Delta#phi;Counts", phiBins, phiMin, phiMax);
    TH1F *h_deltaPhi4pi_totalPt = new TH1F("h_deltaPhi4pi_totalPt", ";#Delta#phi;Counts", phiBins, phiMin, phiMax);
    TH1F *h_deltaPhi4pi_pairPt = new TH1F("h_deltaPhi4pi_pairPt", ";#Delta#phi;Counts", phiBins, phiMin, phiMax);
    TH1F *h_deltaPhi4pi_both = new TH1F("h_deltaPhi4pi_both", ";#Delta#phi;Counts", phiBins, phiMin, phiMax);
    ...
    h_deltaPhi4pi_all->SetStats(0);
    h_deltaPhi4pi_totalPt->SetStats(0);
    h_deltaPhi4pi_pairPt->SetStats(0);
    h_deltaPhi4pi_both->SetStats(0);
    ...
    for (Long64_t i = 0; i < nentries; ++i) {
        ...
        TLorentzVector piA_4pi = boostToTotalRestFrame(selectedPiA, total);
        TLorentzVector piB_4pi = boostToTotalRestFrame(selectedPiB, total);
        double deltaPhi4pi = wrapDeltaPhi(piA_4pi.Phi() - piB_4pi.Phi());
        ...
        h_deltaPhi4pi_all->Fill(deltaPhi4pi);
        if (passTotalPt) {
            h_deltaPhi_totalPt->Fill(deltaPhi);
            h_deltaPhi4pi_totalPt->Fill(deltaPhi4pi);
        }
        if (passPairPt) {
            h_deltaPhi_pairPt->Fill(deltaPhi);
            h_deltaPhi4pi_pairPt->Fill(deltaPhi4pi);
        }
        if (passTotalPt && passPairPt) {
            h_deltaPhi_both->Fill(deltaPhi);
            h_deltaPhi4pi_both->Fill(deltaPhi4pi);
        }
    }
    ...
    FitSummary fitAll4pi;
    FitSummary fitTotal4pi;
    FitSummary fitPair4pi;
    FitSummary fitBoth4pi;
    ...
    saveSinglePlot(h_deltaPhi4pi_all,
                   "c_deltaPhi4pi_A1234_all",
                   "Delta phi in 4#pi rest frame - all events",
                   "All events (4#pi rest frame)",
                   kBlack,
                   outputDir4pi + "/delta_phi_A1234_all_4pi.png",
                   outputDir4pi + "/delta_phi_A1234_all_4pi.pdf",
                   generate_png,
                   generate_pdf,
                   fitAll4pi,
                   "all4pi",
    ...
    saveSinglePlot(h_deltaPhi4pi_totalPt,
                   "c_deltaPhi4pi_A1234_totalPt",
                   "Delta phi in 4#pi rest frame - total pT cut",
                   Form("total p_{T} < %.1f GeV (4#pi rest frame)", kTotalPtCut),
                   kBlue + 2,
                   outputDir4pi + "/delta_phi_A1234_totalPt_4pi.png",
                   outputDir4pi + "/delta_phi_A1234_totalPt_4pi.pdf",
                   generate_png,
                   generate_pdf,
                   fitTotal4pi,
                   "totalPt4pi",
    ...
    saveSinglePlot(h_deltaPhi4pi_pairPt,
                   "c_deltaPhi4pi_A1234_pairPt",
                   "Delta phi in 4#pi rest frame - pair pT cut",
                   Form("pair p_{T} < %.1f GeV (4#pi rest frame)", kPairPtCut),
                   kMagenta + 2,
                   outputDir4pi + "/delta_phi_A1234_pairPt_4pi.png",
                   outputDir4pi + "/delta_phi_A1234_pairPt_4pi.pdf",
                   generate_png,
                   generate_pdf,
                   fitPair4pi,
                   "pairPt4pi",
    ...
    saveSinglePlot(h_deltaPhi4pi_both,
                   "c_deltaPhi4pi_A1234_both",
                   "Delta phi in 4#pi rest frame - both cuts",
                   Form("total p_{T} < %.1f GeV, pair p_{T} < %.1f GeV (4#pi rest frame)", kTotalPtCut, kPairPtCut),
                   kRed + 1,
                   outputDir4pi + "/delta_phi_A1234_both_4pi.png",
                   outputDir4pi + "/delta_phi_A1234_both_4pi.pdf",
                   generate_png,
                   generate_pdf,
                   fitBoth4pi,
                   "bothCuts4pi",
    ...
    std::array<FitSummary, 4> fitSummaries4pi = {fitAll4pi, fitTotal4pi, fitPair4pi, fitBoth4pi};
    saveComparisonPlot(fitSummaries4pi,
                       outputDir4pi + "/delta_phi_A1234_fit_comparison_4pi.png",
                       outputDir4pi + "/delta_phi_A1234_fit_comparison_4pi.pdf",
                       generate_png,
                       generate_pdf);
    ...
    std::ofstream report4pi(outputDir4pi + "/A1234_fit_results_4pi.txt");
    report4pi << "category entries a1 a2 a3 a4 chi2 ndf\n";
    report4pi << "all " << h_deltaPhi4pi_all->GetEntries() << " "
              << fitAll4pi.params[1] << " " << fitAll4pi.params[2] << " " << fitAll4pi.params[3] << " " << fitAll4pi.params[4] << " "
              << fitAll4pi.chi2 << " " << fitAll4pi.ndf << "\n";
    report4pi << "totalPt " << h_deltaPhi4pi_totalPt->GetEntries() << " "
              << fitTotal4pi.params[1] << " " << fitTotal4pi.params[2] << " " << fitTotal4pi.params[3] << " " << fitTotal4pi.params[4] << " "
              << fitTotal4pi.chi2 << " " << fitTotal4pi.ndf << "\n";
    report4pi << "pairPt " << h_deltaPhi4pi_pairPt->GetEntries() << " "
              << fitPair4pi.params[1] << " " << fitPair4pi.params[2] << " " << fitPair4pi.params[3] << " " << fitPair4pi.params[4] << " "
              << fitPair4pi.chi2 << " " << fitPair4pi.ndf << "\n";
    report4pi << "bothCuts " << h_deltaPhi4pi_both->GetEntries() << " "
              << fitBoth4pi.params[1] << " " << fitBoth4pi.params[2] << " " << fitBoth4pi.params[3] << " " << fitBoth4pi.params[4] << " "
              << fitBoth4pi.chi2 << " " << fitBoth4pi.ndf << "\n";
    ...
    h_deltaPhi4pi_all->Write();
    h_deltaPhi4pi_totalPt->Write();
    h_deltaPhi4pi_pairPt->Write();
    h_deltaPhi4pi_both->Write();
    fout->Write();
    fout->Close();
    fin->Close();
